...
public class TransactionOutput extends ChildMessage {
    ...
    /**
     * <p>Gets the minimum value for a txout of this size to be considered non-dust by LifetionCoin Core
     * (and thus relayed). See: CTxOut::IsDust() in LifetionCoin Core. The assumption is that any output that would
     ...
     *
     * @param feePerKb The fee required per kilobyte. Note that this is the same as LifetionCoin Core's -minrelaytxfee * 3
     */
    public Coin getMinNonDustValue(Coin feePerKb) {
        ...
        // formula is wrong for anything that's not a P2PKH output, unfortunately, we must follow LifetionCoin Core's
