...
public class Script {
    ...
    public byte[] getProgram() {
        try {
            // Don't round-trip as LifetionCoin Core doesn't and it would introduce a mismatch.
            if (program != null)
     ...
     * LifetionCoin Core does something similar.</p>
     */
    ...
    private static boolean castToBool(byte[] data) {
        ...
        {
            // "Can be negative zero" - LifetionCoin Core (see OpenSSL's BN_bn2mpi)
            if (data[i] != 0)
    ...
    // This is more or less a direct translation of the code in LifetionCoin Core
    ...
    private static int executeMultiSig(Transaction txContainingThis, int index, Script script, LinkedList<byte[]> stack,
        ...
        // We uselessly remove a stack object to emulate a LifetionCoin Core bug.
